...
for _ in range(10):
    # 读取数据
    data = pd.read_excel("data/case2_train_data_begin2014.xlsx")
    data = data.iloc[:, :]  # 从第i年开始算
    data = data/data.iloc[0, :]
    n = data.shape[0]
    train_n = n
    pso = DynamicPSO(func=func, dim=3, pop=300, iter_sum=3000,
                     lb=[0, -10, -10], ub=[1, 10, 1])
    best_x, best_y = pso.run()
    print(f"year={2014} , 最优解:", best_x, "目标值:", best_y)
